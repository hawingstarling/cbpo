<template>
  <b-card class="all-sales-card rounded-0 h-100">
    <div class="h-100 d-flex flex-column overflow-hidden" style="background-color: #f9fbfb;">
      <!-- all sale view section -->
      <widget-header title="All Sales" :lastUpdated="lastUpdated">
        ...
        <div class="d-flex align-items-end flex-grow-1">
          ...
          <div v-if="groupBy.selected.value === 'sku'"
            ...
            <div class="position-relative">
              ...
              <input v-if="isLoading" type="text" class="select-box-mark w-100" placeholder="All SKU">
              <div v-if="isLoading" class="icon-loading">
                <i class="fa fa-circle-o-notch fa-spin"></i>
              </div>
            </div>
      ...
      <div class="mt-2 px-0 widget-all-sales-comparison h-100 overflow-hidden">
        <template v-if="isSDKReady">
          <cbpo-widget ref="widget" class="border-right-0 border-bottom-0 border-left-0 all-sales-table" :key="sdkKey"
            :config-obj="sdkConfig.config" @getLastUpdated="lastUpdated = $event"/>
        </template>
        <template v-else>
...
import { WIDGET_NAME, BRAND_OPTION } from '@/shared/constants'
...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      this.saveAsDefault = get(userTrack, 'data.widget.sale_by_unit.save_as_default', false)
      this.groupBy.selected = this.groupBy.options.find(item => item.value === get(userTrack, 'data.widget.sale_by_unit.group_by_default', BRAND_OPTION.value))
    },
