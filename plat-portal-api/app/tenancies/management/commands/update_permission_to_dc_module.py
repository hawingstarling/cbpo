...
class Command(BaseCommand):
    help = "Migrate grant access permission dc_inv_summary_view of DC to user client role owner, admin."
    ...
    def handle(self, *args, **options):
        ...
        2. Grant permission DC to user client has role ["OWNER", "ADMIN"]
        :param args:
        :param options:
        :return:
        """
        print("Begin update permission of DC moodule")
        try:
            # 1
            client_ids = ClientModule.objects.filter(client__active=True, module="DC").values_list('client_id',
                                                                                                   flat=True)
            #
            objs = []
            for client_id in client_ids:
                ...
                for user_id in user_ids:
                    ...
                    find = UserModulePermission.objects.filter(
                        ...
                        module="DC",
                        permission="dc_inv_summary_view",
                        user_client=user_client)
                    if not find.exists():
                        obj = UserModulePermission(user_id=user_id, client_id=client_id, module="DC",
                                                   permission="dc_inv_summary_view",
        ...
        print("End update permission of DC moodule")
