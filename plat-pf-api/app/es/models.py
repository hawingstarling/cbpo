...
class ESConfig(TimeStampedModel):
...
class ESClientConfig(TimeStampedModel):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    client = models.ForeignKey(ClientPortal, on_delete=models.CASCADE)
    es = models.ForeignKey(ESConfig, on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.id} - {self.client} - {self.es}"
