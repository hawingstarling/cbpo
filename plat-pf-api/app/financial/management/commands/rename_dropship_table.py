...
class Command(BaseCommand):
    help = "Command rename dropship to warehouse_processing_fee"
    ...
    def handle(self, *args, **options):
        ...
        sql_rename_table_base_template = """
            ...
            BEGIN
              IF EXISTS(SELECT *
                ...
                WHERE table_name='{sale_item_tbl}' and column_name='{col_change}')
              THEN
                  ALTER TABLE "public"."{sale_item_tbl}" RENAME COLUMN "{col_change}" TO "warehouse_processing_fee";
              END IF;
            END $$;
            ...
            BEGIN
              IF EXISTS(SELECT *
                FROM information_schema.columns
                WHERE table_name='{sale_item_tbl}' and column_name='{col_accuracy_change}')
              THEN
                  ALTER TABLE "public"."{sale_item_tbl}" RENAME COLUMN "{col_accuracy_change}" TO "warehouse_processing_fee_accuracy";
              END IF;
            END $$;
            ...
            BEGIN
              IF EXISTS(SELECT *
                FROM information_schema.columns
                WHERE table_name='{sale_item_financial_tbl}' and column_name='{col_change}')
              THEN
                  ALTER TABLE "public"."{sale_item_financial_tbl}" RENAME COLUMN "{col_change}" TO "warehouse_processing_fee";
              END IF;
            END $$;
            ...
            BEGIN
              IF EXISTS(SELECT *
                FROM information_schema.columns
                WHERE table_name='{sale_item_financial_tbl}' and column_name='{col_accuracy_change}')
              THEN
                  ALTER TABLE "public"."{sale_item_financial_tbl}" RENAME COLUMN "{col_accuracy_change}" TO "warehouse_processing_fee_accuracy";
              END IF;
            END $$;
        """
        ...
        sql_rename_table_default = sql_rename_table_default.format(sale_item_tbl='financial_saleitem',
                                                                   ...
                                                                   col_change='dropship_fee',
                                                                   col_accuracy_change='dropship_fee_accuracy')
