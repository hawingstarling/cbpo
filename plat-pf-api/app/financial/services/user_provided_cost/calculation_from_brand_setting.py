...
from app.financial.variable.brand_setting import PO_DROPSHIP_PERCENT_METHOD
from app.financial.variable.job_status import SALE_ITEM_USER_PROVIDED_COST_JOB
...
class UserProvidedCostCalculationFromBrandSetting(IntegrationFinancialBase):
    DT_FILTER_FORMAT = '%Y-%m-%d %H:%M:%S'
    JOB_TYPE = SALE_ITEM_USER_PROVIDED_COST_JOB
    ...
    def __init__(self, client_id: str, flatten: DataFlattenTrack = None, marketplace: str = CHANNEL_DEFAULT, **kwargs):
        ...
        self.__prefetch_brand_user_provided_cost_config()
    ...
    def _get_data_request(self):
        ...
        if not self.is_override:
            cond = cond & Q(user_provided_cost__isnull=True)
        #
    ...
    def __prefetch_brand_user_provided_cost_config(self):
        brand_user_provided_cost_queryset = BrandSetting.objects.tenant_db_for(self.client_id) \
            .filter(
            client_id=self.client_id,
            channel__is_pull_data=True,
            brand__isnull=False,
            add_user_provided_cost__isnull=False
        ).values('client_id', 'channel__id', 'brand__id', 'add_user_provided_method', 'add_user_provided_cost')
        for item in brand_user_provided_cost_queryset:
            self.config_brand_user_provided_cost.update(
                {
    ...
    def __process_data_page(self, sale_items: [SaleItem]):
        for sale_item in sale_items:
            key = f"{sale_item.client_id}-{sale_item.sale.channel_id}-{sale_item.brand_id}"
            try:
                data = self.config_brand_user_provided_cost.get(key, {})
                if not data:
                    continue
                method = data['add_user_provided_method']
                cost = data['add_user_provided_cost']
                assert cost is not None, "User-Provided Cost cannot be None"
                if method == PO_DROPSHIP_PERCENT_METHOD:
                    assert 0 <= cost <= 100, "User-Provided Percentage must have range in [0, 100]"
                    value = (sale_item.sale_charged * cost) / 100
                    value = round_currency(value)
                else:
                    value = cost * sale_item.quantity
                validated_data = dict(
                    user_provided_cost=value
                )
                ...
                logger.debug(f"[{self.__class__.__name__}][{self.client_id}][sale_item_pk={sale_item.pk}] {ex}")
    ...
    def progress(self):
        ...
        if not pages:
            logger.error(
                ...
                f"not found channel sale ids trans for update"
            )
            return
