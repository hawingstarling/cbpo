...
export const parseShortcode = (shortCode) => {
  ...
  let hasContent = (shortCode.match(/\[\/\w+\]/gm) || []).length
  if (hasContent > 1) {
    throw new Error('invalid shortCode: match more then 2 tokens "]". Use only shortcode with this function. Example "[name]teste[/name]" or "[name prop=value]"')
    ...
    switch (mode) {
      ...
      case 'PARSING':
        ...
        if (token === ']') {
          ...
          if (!curAttribute) {
            throw new Error('invalid token: "' + token + '" encountered at ' + match.index)
