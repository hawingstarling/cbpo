...
export class BubbleChartBuilder extends ScatterChartBuilder {
  getBaseConfig() {
    return {
      title: {
        text: null
  ...
  __mappingDataFromDataSource(dataSource, chartConfig) {
    ...
    return chartConfig.charts[0].series.map((item) => {
      const { x, y, z } = mappingColumnNameToAliasNameWithGrouping(
        item,
        chartConfig
      )
      const indexOfY = cols.findIndex((col) => col.name === y)
      const indexOfX = cols.findIndex((col) => col.name === x)
      const indexOfZ = cols.findIndex((col) => col.name === z)
  ...
  __buildTooltip(dataSource, chartConfig) {
    return {
      ...
      headerFormat: '',
      formatter: function() {
        const {x = 0, y = 0, z = 0} = this.point
        const {name, index: seriesIndex, color} = this.series
        const {x: xName, y: yName, z: zName} = get(chartConfig, `charts[0].series[${seriesIndex}].data`, {x: '', y: ''})
        ...
        const columnZ = get(chartConfig, 'columns', []).find((column) => column.name === zName)
        ...
        const xFormatter = columnX.format
          ...
          : null
        const zFormatter = columnZ.format
          ? dataFormatManager.create(columnZ.format, true)
          : null
        ...
        const zTooltip = `<span>${columnZ.displayName || columnZ.name}: ${zFormatter ? zFormatter(z) : z}</span>`

        return `${colorTooltip} <span><b>${name}: </b> <br/><br/> ${xTooltip} <br/> ${yTooltip} <br/> ${zTooltip}</span>`
      }
    }
  }
