...
from plat_import_lib_api.models import AsinTest, RawDataTemporary
...
class LibImportAPITest(APITestCase):
    ...
    def _verify_upload_file(self, rs):
        ...
        #
        raws_queryset = RawDataTemporary.objects.filter(lib_import_id=content['id'])
        #
        self.assertEqual(raws_queryset.count(), self.total_records)
    ...
    def _validate_import(self):
        ...
        #
        self.assertEqual(raws_queryset.count(), self.total_records)
        self.assertEqual(raws_queryset.filter(is_valid=True).count(), 7)
    ...
    def _validate_process_import(self):
        ...
        #
        self.assertEqual(raws_queryset.count(), self.total_records)
        self.assertEqual(raws_queryset.filter(is_complete=True).count(), 7)
