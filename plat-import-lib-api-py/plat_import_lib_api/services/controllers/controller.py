...
from ...models import DataImportTemporary, RawDataTemporary
...
class ImportController(LibImportObject):
    ...
    def __init__(self, lib_import_id: str, action: str, map_config_request: list, **kwargs):
        ...
        self.time_exec_start = time.time()
    ...
    def process(self):
        self.set_status_raws_data_temporary()
        if plat_import_setting.use_queue:
            transaction.on_commit(lambda: self.process_by_action())
        else:
            self.process_by_action()

    def process_by_action(self):
        if self.action == UPLOAD_ACTION:
            self.lib_module.upload(lib_import_id=self.lib_import_id, **self.kwargs)

        elif self.action == VALIDATE_ACTION:
        ...
        elif self.action == PROCESS_ACTION:
            ...
            self.time_exec_end = time.time()
            self.update_process()
        else:
            pass

    def set_status_raws_data_temporary(self):
        if self.action in [VALIDATE_ACTION, PROCESS_ACTION] and self.lib_import.progress == 0:
            RawDataTemporary.objects.filter(lib_import_id=self.lib_import_id).update(status=self.lib_import.status,
