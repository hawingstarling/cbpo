...
from ...static_variable.raw_data_import import RAW_IGNORED_TYPE
...
class BaseModule(object):
    ...
    __VERIFY_RAW_IGNORE__ = False
    __VERIFY_RAW_DELETE__ = False
    ...
    def __init__(self, **kwargs):
        ...
        self.fields_model_compared = self.get_fields_model_compared()
        self.lib_import_instance = self.kwargs.get('lib_import_instance', None)
        #
        self.instance_model_origin = None
    ...
    def fields_model_ignore_compared(self):
        return ['pk', 'is_removed', 'created', 'modified']
    ...
    def find_instance(self, lib_import_id: str, validated_data: dict, **kwargs):
        try:
            ...
            self.instance_model_origin = self.model_objects.get(**filters)
        except Exception as ex:
            logger.error(ex)
            self.instance_model_origin = None
        return self.instance_model_origin

    def get_fields_model_compared(self):
        return [field for field in self.fields_model_accept if field not in self.fields_model_ignore_compared]
    ...
    def handler_verify_raw_process_ignore(self, raw_instance, instance_model_target, created: bool):
        try:
            ...
            assert self.instance_model_origin is not None and not created, "Raw is not accept ignore"
            if model_to_dict(self.instance_model_origin, fields=self.fields_model_compared) == model_to_dict(
                    instance_model_target, fields=self.fields_model_compared):
