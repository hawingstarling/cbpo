...
class SerializerHandle(object):
    ...
    def get_column_config(self, name_detector: bool = False):
        ...
        for field in columns:
            ...
            description = content_type.help_text
            mandatory = content_type.required and not content_type.allow_null
    ...
    def get_field_type(content_type):
        ...
        if isinstance(content_type, DateTimeField):
            return "datetime"
        if isinstance(content_type, DateField):
            return "date"
        # if isinstance(content_type, BooleanField) or isinstance(content_type, NullBooleanField):
        #     return "boolean"
        if isinstance(content_type, BooleanField):
            return "boolean"
        return "string"
